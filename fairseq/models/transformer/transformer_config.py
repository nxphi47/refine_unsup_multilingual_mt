...
class EncDecBaseConfig(FairseqDataclass):
    ...
    layerdrop: float = field(default=0, metadata={"help": "LayerDrop probability", "parse_default": True})
    layers_to_keep: Optional[List[int]] = field(
        default=None, metadata={"help": "which layers to *keep* when pruning", "parse_default": True}
    )
...
class QuantNoiseConfig(FairseqDataclass):
    pq: float = field(
        default=0.0,
        metadata={"help": "iterative PQ quantization noise at training time", "parse_default": True},
    )
    pq_block_size: int = field(
        default=8,
        metadata={"help": "block size of quantization noise at training time", "parse_default": True},
    )
    scalar: float = field(
        default=0.0,
        metadata={
            "help": "scalar quantization noise and scalar quantization at training time", "parse_default": True
        },
    )
...
class TransformerConfig(FairseqDataclass):
    ...
    no_cross_attention: bool = field(
        default=False, metadata={"help": "do not perform cross-attention", "parse_default": True}
    )
    cross_self_attention: bool = field(
        default=False, metadata={"help": "perform cross+self-attention", "parse_default": True}
    )
