...
from fairseq.notifier_utils import maybe_send_email_notification, notify_on_exception
...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        # setup task cache model
        try:
            self.task.setup_cache_model(self)
        except AttributeError as e:
            # skip setup cache model
            pass
    ...
    def should_save_checkpoint_on_current_rank(self) -> bool:
        ...
        if (
            ...
            return True
        # elif self.cfg.distributed_training.save_sharded_state:
        #     return True
        else:
    ...
    def save_sharded_states(self, filename):
        states = {"sharded_states": {}}
        world_size = self.data_parallel_world_size
        # FIXME: if we have actual model parallel or something like megatron, this would behave differently.
        cur_rank = self.data_parallel_rank
        for param_name, param in self.model.named_parameters():
            if getattr(param, "model_parallel", False):
                states['sharded_states'][f'{param_name}'] = param.data
        logger.warning(f'r{cur_rank}: saving sharded states -> {filename}')
        checkpoint_utils.torch_persistent_save(
            states,
            filename,
            async_write=self.cfg.checkpoint.write_checkpoints_asynchronously,
        )
        logger.warning(f'r{cur_rank}: finished saving sharded states -> {filename}')
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            if self.cfg.distributed_training.save_sharded_state:
                rank = self.data_parallel_rank
                assert filename.endswith(".pt"), f'{filename}'
                if self.cfg.distributed_training.load_sharded_state_id >= 0:
                    shard_filename = ".".join(filename.split(".")[:-1] + [f"sharded_states_r{self.cfg.distributed_training.load_sharded_state_id}", "pt"])
                else:
                    shard_filename = ".".join(filename.split(".")[:-1] + [f"sharded_states_r{rank}", "pt"])
                # assert PathManager.exists(shard_filename)
                if PathManager.exists(shard_filename):
                    logger.warning(f'r{rank}: load sharded_states at {shard_filename}')
                    with open(shard_filename, 'rb') as shard_s_f:
                    ...
                    logger.warning(f'r{rank}: DID NOT LOAD sharded_states as file not exists: {shard_filename}')
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                ...
                if "out of memory" in str(e):
                    ...
                    # maybe_send_email_notification("OOM, attempting to recover")
                    ooms += 1
                    self.zero_grad()
                    if self.cuda:
                        torch.cuda.empty_cache()
    ...
    def _check_grad_norms(self, grad_norm):
        ...
        if self._grad_norm_buf is not None:
            ...
            def is_consistent(tensor):
                ...
                diff_consistent = (finite
                    and (max_abs_diff / (tensor[0] + 1e-6) < 1e-6).all())
                return (
                    diff_consistent
                    or
                    (finite and self.cfg.distributed_training.save_sharded_state)
                    or
                    (self.cfg.common.amp and not finite)
