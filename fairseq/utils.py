...
def overwrite_sharded_states(sharded_states, model_state):
    # logger.info(f'Load reassign sharded states: {len(sharded_states)=} --> model')
    for k, v in sharded_states.items():
        sk = k
        while sk.startswith("module."):
            sk = sk[len("module."):]
        mk = sk
        model_state[mk] = v
